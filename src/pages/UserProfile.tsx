...
function UserProfile() {
  ...
  if (!userProfile) return <div className="text-center mt-5">Cargando perfil...</div>;
    ...
    <div className="container profile-page mt-4">
          ...
          <div className="profile-user flex-column align-items-center text-center">
            ...
            <div className="profile-user-info w-100">
              <h2 className="profile-name d-flex justify-content-center align-items-center gap-2">
                @{userProfile.nickname} { <LuBadgeCheck size={22} color="var(--accent)" />}
              </h2>
              <p className="profile-stats justify-content-center">
                <strong>{misPosts.length}</strong> publicaciones • <strong>{ 0}</strong> seguidores
              </p>
              ...
              <p className="profile-bio mx-auto" style={{ maxWidth: "500px" }}>{ "Todavía no escribió una biografía."}</p>
            </div>
          </div>
      ...
      {cargando ? <div className="text-center py-5">Cargando...</div> : (
        activeTab === "posts" ? (
          ...
          misComentarios.length === 0 ? <p className="text-center">No ha hecho comentarios aún.</p> : misComentarios.map(c => (
